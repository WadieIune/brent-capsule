...
\title{Detección, persistencia y aplicación al riesgo de los canales de tendencia
en el Brent:\\
del chartismo multiactivo al detector solo-precio, con análisis de supervivencia
del régimen y VaR de cartera}
\author{}
\date{}
...
Una segunda pata modela la \textbf{persistencia} del régimen mediante análisis de
supervivencia sobre 590 episodios de canal: la vida del túnel es ordenable
(C-index 0.664 con XGBoost-AFT, estable en \emph{walk-forward}) y su dirección de
ruptura predecible (AUC 0.766), pero el \emph{backtest} de la estrategia
resultante vuelve a fracasar (DSR 0.014) —acertar el 67.8\% de las direcciones de
ruptura no basta, porque predecir \emph{por qué banda} sale el precio no es
predecir el beneficio—, replicando el veredicto de la primera pata por una vía
independiente. Una tercera pata lleva ambas señales a la medición de riesgo sobre
una cartera de seis materias primas y arroja un resultado igualmente negativo,
documentado junto con los tres defectos de datos que invalidaron una conclusión
previa: la fragilidad del canal \emph{no} anticipa la cola —en los días de mayor
fragilidad ocurren cero excepciones, frente a un \emph{lift} de 3.19 de la
volatilidad EWMA, con correlación $-0.12$ entre ambas señales— y una constante
del mismo nivel medio iguala al \emph{overlay}. Quien sí mejora el VaR es el
filtrado de volatilidad: cobertura del 1.005\%, corrige el agrupamiento de
excepciones del VaR histórico y ahorra un 1.8\% de capital. La geometría del
canal sí aporta valor medible en otros dos ámbitos: la calidad de dato, donde
detecta defectos invisibles a un control convencional de cola y corrige 74.4
puntos porcentuales de distorsión en la contribución al riesgo, y la descripción
interpretable del régimen. El documento cierra con el marco
de aplicación en riesgos: control de capital en cuatro capas y un cuadro de mando
donde el sistema actúa como descriptor de régimen, nunca como señal de inversión.
\end{abstract}
...
La intuición de este resultado se aprecia mejor sobre el propio precio
(Figura~\ref{fig:canalvscross}): en una ventana con un canal ascendente nítido,
la geometría del túnel es evidente en la trayectoria del Brent, mientras que los
activos históricamente correlacionados (EURUSD, DAX, oro) recorren la misma
ventana sin reproducir esa forma. La correlación opera sobre los retornos, no
sobre la geometría local del nivel; por eso una rama alimentada solo con
\emph{cross-asset} no puede ``ver'' el canal.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{canal_vs_crossasset.pdf}
    \caption{La geometría del canal es una propiedad local del precio del Brent.
    Izquierda: túnel ascendente ajustado sobre la ventana del Brent. Derecha: los
    activos correlacionados sobre la misma ventana no llevan esa estructura.}
    \label{fig:canalvscross}
\end{figure}
...
La lectura operativa se ve con claridad al proyectar el régimen detectado sobre
el precio real (Figura~\ref{fig:canaloos}): el detector marca el túnel ascendente
mientras el Brent sube hacia su máximo de abril y, tras el giro, identifica el
túnel descendente a lo largo de toda la caída hasta junio, con los aciertos
concentrados en las pendientes limpias y los pocos fallos en la transición
volátil de mediados de abril. Es exactamente la forma en que una dirección de
riesgos consumiría la señal: un descriptor del régimen vigente, no una predicción
del siguiente movimiento.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{canal_oos_2026.pdf}
    \caption{Uso operativo del detector sobre el precio real del Brent (FRED) en
    el trimestre out-of-sample. Túnel ascendente (verde) sobre la subida hasta el
    máximo de abril; túnel descendente (rojo) sobre la caída posterior; el fondo
    sombrea el régimen predicho día a día y los marcadores señalan aciertos y
    fallos frente a la etiqueta geométrica.}
    \label{fig:canaloos}
\end{figure}
...
Antes del detalle cuantitativo conviene ver qué ``dibuja'' realmente el sistema.
La Figura~\ref{fig:galeria} muestra tres ventanas reales del Brent con el túnel
que el algoritmo detecta: un canal ascendente, uno descendente y un rango
lateral, cada uno con la recta central y las paralelas que un chartista trazaría
a mano. La calidad del ajuste sobre casos reales es la contrapartida visual de
los AUC elevados.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.99\textwidth]{canal_galeria.pdf}
    \caption{Galería de patrones reales del Brent con el túnel detectado por el
    sistema: canal ascendente (oct-2021), canal descendente (jul--ago 2024) y
    rango sin dirección (sep-2025). La etiqueta geométrica y su confianza figuran
    en cada panel.}
    \label{fig:galeria}
\end{figure}
...
% =============================================================================
\section{Segunda pata: ¿cuánto vive el canal? Análisis de supervivencia}
\label{sec:parte2}
% =============================================================================
La primera pata responde \emph{qué} geometría dibuja el precio hoy. Para una
dirección de riesgos, la pregunta operativa inmediata es la siguiente:
\emph{¿cuánto durará ese régimen?} Un canal detectado con AUC 0.97 es un
descriptor útil solo si se sabe si aguantará una sesión o un trimestre —de ello
dependen el horizonte de liquidez, la frecuencia de revisión de límites y la
vigencia de cualquier escenario construido sobre él.

\subsection{Episodios y definición de ruptura}
Cada detección abre un \emph{episodio}: se congela la geometría (recta central
por MCO sobre la ventana de 32 días y bandas paralelas a $\pm m\,\sigma_
\varepsilon$, con $m=2$), se proyecta hacia delante y se declara \textbf{ruptura}
cuando el cierre supera la banda proyectada más una tolerancia de un ATR durante
$c$ sesiones consecutivas. La duración $T$ es el número de sesiones hasta la
ruptura; los episodios vivos al cierre de la muestra quedan censurados por la
derecha. Los episodios son no solapados.

Dos decisiones metodológicas condicionan todo el bloque y por eso se explicitan:

\begin{enumerate}
  \item \textbf{Confirmación $c=2$.} Con $c=1$, el 23\% de los episodios
    ``rompen'' el mismo día de la detección: son cruces de banda por ruido de
    microestructura, no cambios de régimen. Exigir dos cierres consecutivos fuera
    de banda los elimina por completo y eleva la duración mediana de 5 a
    \textbf{10} sesiones. La tabla de sensibilidad completa sobre la rejilla
    $m \times \mathrm{tol} \times c$ acompaña a los resultados.
  \item \textbf{Censura administrativa.} Un episodio detectado antes del corte
    temporal pero que rompe después usaría información futura —su duración y su
    dirección de ruptura— para entrenar. En cada corte se censura el episodio en
    la frontera: solo se sabe que sobrevivió hasta ahí. Sin esta corrección hay
    fuga temporal en el conjunto de entrenamiento.
\end{enumerate}

\subsection{Resultados}
Sobre 590 episodios (serie FRED 1987--2026) se ajustan Kaplan--Meier (baseline
marginal), Cox PH, \emph{Random Survival Forest} y XGBoost-AFT, evaluados con
\emph{walk-forward} sobre seis orígenes temporales expansivos
(Figura~\ref{fig:parte2}).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.99\textwidth]{part2_supervivencia.pdf}
    \caption{Segunda pata. Izquierda: curva de supervivencia del canal
    (Kaplan--Meier); la mediana de vida es de 10 sesiones y
    $P(T>20)\approx0.24$. Centro: C-index \emph{walk-forward} de los tres
    modelos con covariables frente al azar. Derecha: backtest económico de la
    estrategia implícita en la dirección de ruptura.}
    \label{fig:parte2}
\end{figure}
...
\centering
\footnotesize
\caption{Segunda pata: discriminación, calibración y valor económico.}
\label{tab:parte2}
\begin{tabularx}{\textwidth}{p{3.5cm}p{4.4cm}L}
\toprule
\textbf{Pregunta} & \textbf{Resultado} & \textbf{Lectura} \\
\midrule
Q2 · discriminación & XGB-AFT C-index $\mathbf{0.664 \pm 0.007}$ (RSF 0.624; Cox 0.558; azar 0.500) &
La vida del canal \textbf{es} ordenable: se identifica qué canales son frágiles \\
Q2 · calibración & MAE de $P(T{>}k)$: KM \textbf{0.019} $<$ Cox 0.034 $<$ XGB-AFT 0.042 &
Las covariables aportan \emph{ranking}, no mejor probabilidad absoluta \\
Q3 · dirección de ruptura & AUC $\mathbf{0.766 \pm 0.050}$ &
La banda por la que sale el precio es predecible \\
Q4 · valor económico & Sharpe $-0.48$; retorno $-78\%$ frente a \emph{buy\&hold} $+88\%$; \textbf{DSR 0.014}; PBO 0.35 &
\textbf{Sin ventaja explotable} \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{El resultado central de esta pata}
El modelo acierta el \textbf{67.8\%} de las direcciones de ruptura y, aun así, la
estrategia que las opera \textbf{pierde dinero}. No hay contradicción:
\emph{predecir por qué banda sale el precio no es predecir el beneficio}. La
banda superior de un canal ascendente sube con la pendiente, de modo que el
precio puede subir en términos absolutos y aun así salir por abajo; y la
distribución de pagos es asimétrica, con pérdidas grandes en el 32\% de fallos
que anulan las ganancias del 68\% de aciertos. El \emph{Deflated Sharpe Ratio} de
0.014 confirma que el resultado no sobrevive a la corrección por número de
configuraciones probadas.

Este hallazgo, obtenido por una vía metodológicamente independiente (análisis de
supervivencia sobre episodios, no clasificación de imágenes), \textbf{reproduce}
el veredicto de la primera pata (DSR $\approx 0$) y por tanto refuerza la
coherencia del trabajo en lugar de tensionarla.

Consecuencia de diseño para las aplicaciones: el modelo se emplea como
\textbf{ordenador de fragilidad} —qué canal aguantará menos— mientras que el
\emph{nivel} absoluto de $P(T>k)$ se ancla en Kaplan--Meier, que es lo mejor
calibrado.

% =============================================================================
\section{Tercera pata: aplicaciones a la medición de riesgo}
\label{sec:parte3}
% =============================================================================
Si el régimen de un activo se caracteriza con su propio precio (primera pata) y
su persistencia es estimable (segunda), la consecuencia natural es que ambas
señales alimenten la medición de riesgo. Esta sección lo contrasta sobre el
\emph{Value at Risk}, primero en un activo y después en cartera.

\subsection{VaR predicho: diseño y atribución}
Se comparan tres estimadores del VaR a un día al 99\%:

\begin{itemize}
  \item \textbf{Histórico} (baseline débil): cuantil empírico de los retornos en
    ventana móvil de 250 sesiones.
  \item \textbf{FHS-EWMA} (baseline \emph{fuerte}): simulación histórica filtrada
    por volatilidad EWMA ($\lambda=0.94$), \textbf{sin información de canal}.
  \item \textbf{Predicho}: FHS-EWMA más un \emph{add-on} de cola proporcional a
    la fragilidad del régimen, $1-P(T>k)$, tomada de la segunda pata.
\end{itemize}

Incluir el baseline fuerte es deliberado: comparar únicamente contra el VaR
histórico atribuiría al canal una mejora que el simple filtrado de volatilidad ya
produce. La atribución honesta exige aislar ambos efectos.

\subsection{Tres defectos de datos que invalidaron una conclusión previa}
\label{sec:defectos}
Una versión anterior de este análisis concluía que el \emph{add-on} de canal
mejoraba la cobertura del VaR. Una revisión posterior identificó tres defectos
que invalidaban esa conclusión. Se documentan porque el error, y el modo en que se
detecta, forman parte del resultado metodológico.

\begin{enumerate}
  \item \textbf{Precio negativo del WTI.} El 20 de abril de 2020 el WTI liquidó a
    $-37{,}63$ USD. La rutina de log-retornos acotaba el precio a $10^{-9}$,
    generando dos retornos artificiales de $|r|\approx 23$ ($\pm2300\%$) que
    dominaban la matriz de covarianzas: el WTI aparecía con el \textbf{98{,}4\%}
    de la contribución al riesgo de la cartera en lugar del 24\% que le
    corresponde, e inflaba la volatilidad EWMA durante meses.
  \item \textbf{Calendario en lugar de días hábiles.} La matriz \emph{wide} está
    en frecuencia de calendario con \emph{forward-fill}: 365 observaciones al año
    y aproximadamente un \textbf{32\% de retornos exactamente nulos}. Esto sesga
    el cuantil empírico, contamina el test de independencia —un día de retorno
    nulo no puede ser excepción— y desescala el semáforo de Basilea, que cuenta
    250 \emph{sesiones de negociación}. Corregido: 4.680 sesiones a 251{,}6
    observaciones anuales.
  \item \textbf{Ausencia de control de nivel.} Sin comparar contra una
    \emph{constante} con el mismo VaR medio es imposible separar si el
    \emph{overlay} mejora por \emph{timing} —reacciona al régimen— o simplemente
    por \emph{nivel}, por ser más conservador.
\end{enumerate}

\subsection{Cartera multi-commodity: resultado corregido}
La tesis solo-precio implica que el tratamiento se replica activo por activo sin
un modelo multivariante de factores. Se construye una cartera equiponderada de
seis materias primas —Brent, WTI, oro, plata, cobre y gas natural— donde
\emph{cada activo} aporta su fragilidad estimada exclusivamente con su serie de
precios. El \emph{backtest} cubre 1.393 sesiones hábiles (2020-08 a 2026-03) con
Kupiec, Christoffersen, semáforo de Basilea y el contraste DQ de
Engle--Manganelli (Tabla~\ref{tab:parte3}, Figura~\ref{fig:parte3}).

\begin{table}[H]
\centering
\footnotesize
\caption{VaR 99\% de la cartera multi-commodity (datos corregidos).}
\label{tab:parte3}
\begin{tabular}{lrrrr}
\toprule
\textbf{Estimador} & \textbf{Cobertura} & \textbf{Kupiec $p$} & \textbf{Christoffersen $p$} & \textbf{Capital} \\
\midrule
Histórico & 1.292\% & 0.29 & \textbf{0.0011} (rechaza) & 15.01\% \\
\textbf{FHS-EWMA (sin canal)} & \textbf{1.005\%} & \textbf{0.985} & 0.128 & \textbf{14.74\%} $(-1{,}8\%)$ \\
Predicho (con canal) & 0.933\% & 0.800 & 0.108 & 15.81\% $(+5{,}3\%)$ \\
Constante $\times1{,}073$ (control) & 1.005\% & 0.985 & 0.128 & 15.81\% \\
\midrule
\multicolumn{5}{l}{\footnotesize Objetivo: 1.00\%. Capital $= k\cdot\mathrm{VaR}$ con el multiplicador de la peor ventana de 250 sesiones.} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.99\textwidth]{part3_portfolio_var.pdf}
    \caption{Tercera pata, resultado corregido. El VaR histórico agrupa las
    excepciones y el test de independencia lo rechaza; el filtrado de volatilidad
    lo corrige. El tercer panel es el contraste decisivo: en los días de mayor
    fragilidad de canal ocurren \emph{cero} excepciones, mientras la volatilidad
    EWMA las concentra con un \emph{lift} de 3{,}19.}
    \label{fig:parte3}
\end{figure}

\subsection{El canal no anticipa la cola}
La ablación lo demuestra por tres vías independientes:

\begin{itemize}
  \item \textbf{\emph{Lift}}: en el quintil de mayor fragilidad de canal se
    concentran \emph{cero} excepciones ($\mathrm{lift}@q_{80}=\mathbf{0{,}00}$ en
    cartera; $0{,}997$ en el activo aislado), frente a un \emph{lift} de
    $\mathbf{3{,}19}$ de la volatilidad EWMA.
  \item \textbf{Correlación}: $\mathrm{corr}(\text{fragilidad},\sigma)=-0{,}12$.
    El canal se comprime cuando hay calma y las excepciones ocurren en
    volatilidad alta: la señal apunta en sentido \emph{contrario} al que
    necesitaría un estimador de cola.
  \item \textbf{Control de nivel}: una constante $\times1{,}071$ con idéntico VaR
    medio iguala al \emph{overlay} en todas las métricas, luego su mejora
    aparente era un efecto de \textbf{nivel}, no de régimen.
\end{itemize}

Quien sí cumple el objetivo —menos excepciones \emph{y} menos capital— es el
\textbf{FHS-EWMA}: cobertura 1{,}005\% (Kupiec $p=0{,}985$), corrige el
agrupamiento del histórico ($p$ de $0{,}0011$ a $0{,}128$) y ahorra un
$\mathbf{1{,}8\%}$ de capital. Una capa de alertas \emph{discretas} sobre el VaR
histórico tampoco lo consigue ($+0{,}2\%$): la patología del histórico es de
agrupamiento y se corrige con escalado \emph{continuo} de volatilidad, no con
saltos discretos.

Se reporta igualmente un \emph{caveat}: el contraste DQ de Engle--Manganelli
rechaza a \textbf{todos} los estimadores, incluido el mejor. Ninguno captura por
completo la dinámica de la cola.

Este resultado es coherente con las otras dos patas y completa el argumento
central del trabajo: la geometría del canal \textbf{describe el régimen} pero no
anticipa la dirección (primera pata), ni el beneficio (segunda), ni la cola
(tercera).

\subsection{Dónde sí aporta valor la geometría}
El mismo conjunto de experimentos identifica dos ámbitos con mejora medible.

\paragraph{Calidad de dato: qué aporta exactamente, y qué no.}
\label{par:dq}
Conviene ser preciso con la atribución, porque es donde un revisor mirará
primero. El control de calidad de dato de este trabajo \textbf{no utiliza la red
convolucional}: se apoya en una regresión lineal móvil sobre ventanas de 32
sesiones con bandas de residuo, un ATR y una regla de rachas. La verificación es
inmediata —la ruta completa de calidad de dato no carga ningún módulo de
\emph{deep learning}— y se explicita aquí para que la contribución no se lea como
lo que no es. La red convolucional pertenece a la primera pata, donde se demostró
que reconoce la geometría con AUC 0.97 y que \emph{eso no es monetizable}.

La comparación pertinente es, por tanto, con una biblioteca convencional de
calidad de dato: valores repetidos, rachas de repetidos, umbral de tres
desviaciones sobre los retornos y control de precios no positivos. Sobre la
inyección sintética con verdad conocida (Tabla~\ref{tab:dqvsestandar}) el
resultado es matizado, y conviene enunciarlo sin adornos.

\begin{table}[H]
\centering
\footnotesize
\caption{Control geométrico frente a una biblioteca convencional de calidad de
dato. AUC-PR medio sobre 5 familias $\times$ 3 tasas de corrupción $\times$ 10
semillas, con verdad de referencia conocida.}
\label{tab:dqvsestandar}
\begin{tabular}{lrrr}
\toprule
\textbf{Familia de defecto} & \textbf{Geométrico} & \textbf{Convencional} & \textbf{Dif.} \\
\midrule
Relleno / precio congelado & 0.945 & 0.879 & $+0.066$ \\
Salto reversible (\emph{fat finger}) & \textbf{0.634} & 0.022 & $\mathbf{+0.612}$ \\
Salto persistente de nivel & 0.079 & 0.022 & $+0.057$ \\
Precio no positivo & 1.000 & 0.972 & $+0.028$ \\
Desfase de calendario & 0.018 & \textbf{0.321} & $\mathbf{-0.303}$ \\
\midrule
...
En repetidos y precios no positivos el empate es sustancial: para eso bastan una
comparación de igualdad y un control de signo, sin geometría alguna. En el
desfase de calendario \textbf{gana} el control convencional. En el salto
persistente fallan ambos. La única ventaja sustancial del control geométrico está
en el \textbf{salto reversible}, y por un motivo identificable: un umbral de tres
desviaciones global sufre \emph{enmascaramiento} —los propios saltos inflan la
desviación típica hasta que dejan de superar el umbral— y además marca dos días,
el salto y su reversión, por un único defecto; la regla geométrica utiliza la
firma «salta y vuelve», que sí es información de forma. Frente a estadística
robusta la ventaja se estrecha todavía más: 0.945 contra 0.823 de un detector de
rachas en el relleno, y 0.518 contra 0.367 de Hampel/MAD en el salto reversible.

Lo que sí queda establecido, y no depende de qué detector se emplee, es la
\textbf{ceguera estructural} de los controles basados en la cola de la
distribución: Hampel, Tukey e \emph{isolation forest} obtienen un recall de
exactamente \textbf{0.00} sobre el precio congelado y el precio no positivo,
porque un retorno nulo nunca es un valor atípico de cola. No es un problema de
calibración sino de construcción, y muchos controles desplegados en producción
son de ese tipo.

Los dos defectos reales del panel —el precio negativo del WTI y el relleno de
calendario, con 1.145 marcas \emph{stale} de las que el 88{,}6\% caen en fin de
semana— caen justo en ese punto ciego. Depurarlos corrige \textbf{74{,}4 puntos
porcentuales} de distorsión en la contribución al riesgo (la del WTI baja de
$0{,}985$ a $0{,}240$) y elimina el 31{,}1\% de retornos nulos.

\paragraph{La consecuencia en capital.}
\label{par:capital}
La contribución práctica de este trabajo no es un detector nuevo, sino
\textbf{cuantificar en capital regulatorio y en observabilidad la consecuencia de
ese punto ciego}. Calculando el VaR y el capital de la cartera por dos vías
idénticas salvo en la depuración del dato:

\begin{table}[H]
\centering
\footnotesize
\caption{Impacto de la calidad del dato sobre el capital por riesgo de mercado.}
\label{tab:dqcapital}
\begin{tabular}{lrrr}
\toprule
& \textbf{Sin depurar} & \textbf{Depurado} & \textbf{Variación} \\
\midrule
VaR medio (99\%, histórico) & 3.562\% & 4.114\% & $+15{,}5\%$ \\
Capital $k\cdot\mathrm{VaR}$ & 10.69\% & 12.34\% & $+15{,}5\%$ \\
\bottomrule
\end{tabular}
\end{table}

Es decir, \textbf{consumir el panel tal y como se recibe subestima el capital por
riesgo de mercado en un 13{,}4\%}. Por la vía de FRTB el efecto es análogo: bajo
el \emph{Risk Factor Eligibility Test} (MAR31.12) un factor es modelizable si
acumula al menos 24 observaciones anuales sin huecos de 90 días con menos de
cuatro, y un precio arrastrado por \emph{forward-fill} no es una observación
aunque el recuento ingenuo lo cuente como tal. La observabilidad declarada
resulta estar \textbf{inflada en un factor de 1{,}46} de media: en la peor
ventana de 90 días el Brent declara 90 observaciones cuando solo 52 son reales, y
se registran rachas de hasta 16 sesiones con el precio congelado. En esta cartera
—seis materias primas líquidas— ningún factor deja de ser modelizable, de modo
que el efecto es potencial y no efectivo; pero el mecanismo queda demostrado y es
exactamente donde el RFET resulta vinculante, porque un factor próximo al umbral
pasaría por modelizable sin serlo y su capital se calcularía por IMA en lugar de
por SES.

Este análisis es \textbf{confirmatorio y no pre-registrado}: el efecto sobre el
capital se observó al corregir los defectos de datos descritos en la
Sección~\ref{sec:defectos}, y se declara como tal.

\paragraph{Descripción interpretable del régimen.} Una cadena de Markov de cuatro
estados (ascendente/descendente $\times$ borde/centro) supera al modelo i.i.d. en
$+0{,}61$ de log-verosimilitud por observación, con permanencias esperadas de
$3{,}2$ a $3{,}7$ sesiones; el contraste de homogeneidad del \emph{hazard} por
edad se rechaza ($\chi^2=13{,}34$, $p=0{,}020$), lo que indica dependencia
temporal y recomienda un modelo semi-Markov. Sobre la anticipación de la ruptura
a cinco sesiones, de todos los predictores contrastados solo la \textbf{distancia
al borde} del canal aporta información (AUC $0{,}677$ frente a $0{,}496$ del
modelo actuarial y $0{,}482$ de la volatilidad), con un \emph{lead time} mediano
de seis sesiones.

\paragraph{Capital bajo FRTB.} El módulo de capital calcula el \emph{Expected
Shortfall} al 97{,}5\% escalado al horizonte de liquidez prescrito (MAR33.12) y
el IMCC, con un periodo de estrés identificado en 2008-03 a 2009-03. La
comparación del cambio de modelo arroja una caída de capital del $57{,}1\%$
frente a Basilea 2.5, si bien el cálculo es \textbf{parcial} —omite SES, DRC y
RRAO, y la caída refleja sobre todo que Basilea 2.5 suma VaR y sVaR con $k=3$
frente a un único ES estresado con $m_c=1{,}5$—. La vida mediana del canal
(11 sesiones) se emplea únicamente como evidencia de apoyo del horizonte, nunca
en sustitución del valor prescrito.

\subsection{Una atribución falsa detectada por auditoría}
\label{sec:auditvol}
La regla que el proyecto se impuso tras el episodio del VaR —ningún resultado
positivo se sostiene sin un \emph{baseline} fuerte— se aplicó también a los
\texttt{accept} propios. El más expuesto era el módulo que relaciona la
compresión del canal con la expansión de la volatilidad realizada: se había
publicado con AUC $0{,}716$ contrastado únicamente frente a la tasa base
($\mathrm{AUC}=0{,}5$), sin compararse jamás con un modelo de volatilidad.

La sospecha tenía fundamento estructural. De las once variables que el
experimento denomina ``geometría'', \textbf{cuatro son medidas de volatilidad}:
la desviación de los últimos veinte retornos, el ATR relativo, la dispersión de
los residuos en torno a la recta del canal y la anchura de banda, que es esa
misma dispersión reescalada. Y el objetivo —si la volatilidad futura superará a
la actual— es en buena medida una pregunta sobre reversión a la media de la
volatilidad.

La auditoría contrasta el modelo publicado con volatilidad realizada, EWMA
($\lambda=0{,}94$) y GARCH(1,1) —parámetros estimados solo en entrenamiento—
manteniendo idénticos el objetivo, la partición, el clasificador y la
estandarización (Tabla~\ref{tab:auditvol}).

\begin{table}[H]
\centering
\footnotesize
\caption{Auditoría del módulo de volatilidad: descomposición por conjunto de variables.}
\label{tab:auditvol}
\begin{tabular}{lrl}
\toprule
\textbf{Modelo} & \textbf{AUC} & \textbf{Contenido} \\
\midrule
\textbf{Solo proxies de volatilidad} & \textbf{0.723} & \textbf{las 4 medidas de volatilidad, sin forma alguna} \\
Geometría publicada & 0.716 & las 11 variables tal y como se publicaron \\
Mejor volatilidad + forma & 0.631 & prueba incremental \\
Volatilidad realizada & 0.625 & nivel y posición relativa \\
Forma pura & 0.608 & geometría \emph{sin} los 4 proxies \\
GARCH(1,1) & 0.566 & parámetros solo de entrenamiento \\
EWMA ($\lambda=0.94$) & 0.560 & — \\
\bottomrule
\end{tabular}
\end{table}

Tres contrastes \emph{bootstrap} ($B=3000$) cierran la atribución. Frente a la
volatilidad realizada, el modelo publicado gana ($\Delta\mathrm{AUC}=+0{,}091$;
IC 95\% $[+0{,}053,+0{,}127]$), de modo que \textbf{supera el umbral
pre-registrado y conserva su decisión}. Pero frente a \emph{sus propios proxies
de volatilidad} no gana ($\Delta\mathrm{AUC}=-0{,}009$; IC 95\% por bloques
temporales de 63 sesiones $[-0{,}028,+0{,}011]$), de modo que \textbf{no se
establece} que la forma aporte por encima de la volatilidad que las propias
variables contienen. El matiz importa: un intervalo que contiene el cero es
compatible con una aportación nula o pequeña en cualquier sentido, y no prueba
equivalencia. Estas cifras incorporan tres correcciones señaladas en auditoría
cruzada: purga de las diez sesiones cuyas etiquetas rebasaban el corte, elección
del \emph{baseline} en validación en lugar de en test, y remuestreo por bloques
en lugar de i.i.d.

La conclusión es que el módulo funciona, pero \textbf{no por el motivo que se le
atribuía}. Su capacidad no queda establecido que proceda de la geometría del canal, sino que es compatible con proceder de contener una
\emph{representación multi-medida de la volatilidad} —realizada, rango medio y
dispersión \emph{detrended} en torno a la tendencia local— que resulta mejor
predictor de la expansión que EWMA o GARCH(1,1) basados en una sola medida. El
hallazgo conserva interés, pero pertenece a la \textbf{medición de volatilidad} y
no al chartismo: la afirmación de que la compresión del canal anticipa la
expansión de la volatilidad queda refutada.

Las once aplicaciones se ejecutan bajo un \emph{harness} reproducible que registra
por corrida un manifiesto con identificadores, configuración, semilla, comando,
entorno, métricas, baseline y decisión, de acuerdo con la disciplina de
\emph{harness engineering} adoptada en el proyecto. Las decisiones negativas
(\texttt{reject}) se publican con el mismo detalle que las positivas.
...
% =============================================================================
\section{Marco de aplicación en riesgos: control de capital en cuatro capas}
\label{sec:riskframework}
% =============================================================================
El mandato de una dirección de riesgos no es predecir la dirección del mercado,
sino \textbf{controlar el capital y sobrevivir a los regímenes}. Los hallazgos
empíricos de las secciones anteriores delimitan con precisión qué puede y qué no
puede hacer el sistema dentro de ese mandato
(Tabla~\ref{tab:evidencia-riesgo}).

\begin{table}[H]
\centering
\footnotesize
\caption{De la evidencia empírica a la implicación de riesgo.}
\label{tab:evidencia-riesgo}
\begin{tabularx}{\textwidth}{L L}
\toprule
\textbf{Hallazgo empírico} & \textbf{Implicación para riesgo} \\
\midrule
Dirección del Brent no predecible (corr $\approx 0$, \emph{hit} $\approx$ tasa base) &
Prohibido dimensionar capital por ``señal direccional'' del modelo \\
El chartismo no pasa el \emph{backtest} (DSR $\approx 0$, pierde frente a
\emph{buy\&hold}, MaxDD $-59\%$) &
El patrón no justifica subir tamaño; sirve de \emph{challenge} al \emph{desk} \\
Volatilidad sí predecible (persistencia, correlación 0.32--0.45) &
Palanca central del marco: \emph{sizing} y límites en función del riesgo \\
Detección de canal fiable y estable (AUC 0.97, sin decaimiento a 12 meses) &
Descriptor de régimen, no \emph{alpha} \\
El \emph{cross-asset} no predice dirección pero define la correlación &
Agregación del riesgo de cartera, no el \emph{trade} individual \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Las cuatro capas de protección del capital}
Cada capa acota una dimensión de riesgo; la Capa~3 es la única alimentada por el
detector, y exclusivamente como \emph{descriptor}:

\begin{center}
\begin{tikzpicture}[font=\small]
\tikzstyle{layer}=[rounded corners=6pt, minimum width=13cm, minimum height=1.12cm, align=center, draw]
\node[layer, fill=cBlue, draw=cBlueD, text=cBlueD] (l4)
  {\textbf{Capa 4 · Riesgo de cartera y correlación}\\ VaR/ES · \emph{netting} largo-corto · correlación móvil};
\node[layer, fill=cTeal, draw=cTealD, text=cTealD, below=3mm of l4, minimum width=11.5cm] (l3)
  {\textbf{Capa 3 · Overlay de régimen} \; $\blacktriangleleft$ detector de canal\\ probabilidad de canal como descriptor, no como señal};
\node[layer, fill=cBlue, draw=cBlueD, text=cBlueD, below=3mm of l3, minimum width=10cm] (l2)
  {\textbf{Capa 2 · Límites duros}\\ bruto/neto · concentración · \emph{drawdown} y \emph{stop}};
\node[layer, fill=cBlue, draw=cBlueD, text=cBlueD, below=3mm of l2, minimum width=8.5cm] (l1)
  {\textbf{Capa 1 · Sizing por riesgo}\\ \emph{vol targeting} · tamaño $\propto 1/\sigma$};
\node[layer, fill=cGray, draw=cGrayD, text=cGrayD, below=3mm of l1, minimum width=6cm, minimum height=0.9cm] (c)
  {\textbf{Capital} · presupuesto de riesgo};
\end{tikzpicture}
\end{center}

\begin{description}
\item[Capa 1 --- Sizing por riesgo (diario).] Como la dirección no es
  \emph{forecastable} pero la volatilidad sí, cada posición (larga o corta) se
  dimensiona inversamente a su volatilidad prevista:
  $w_i = (\text{riesgo objetivo}/\sigma_i)/\sum_j (1/\sigma_j)$ (\emph{risk
  parity}), con $\sigma_i$ estimada por EWMA ($\lambda=0.94$) o GARCH(1,1). El
  objetivo de volatilidad de cartera es fijo: si sube la volatilidad, el tamaño
  baja automáticamente.
\item[Capa 2 --- Límites duros (intradía).] Topes de exposición bruta y neta (el
  sesgo direccional debe estar acotado: no hay \emph{edge}), concentración por
  activo y por sector (energía/metales/agrícolas), y \emph{drawdown}/\emph{stop}
  con reducción automática de riesgo. Riesgo de curva/\emph{roll} si se opera con
  futuros.
\item[Capa 3 --- Overlay de régimen (diario).] El detector etiqueta el régimen
  (ascendente/descendente/rango) en el cuadro de mando. \emph{Nunca} aumenta
  capital; solo marca posiciones contra-régimen para revisión y dispara el
  recálculo de riesgo.
\item[Capa 4 --- Cartera y correlación (diario).] VaR/ES, \emph{netting} real de
  largos y cortos, correlación en ventana móvil y ratio de diversificación:
  detectar cuándo ``posiciones diversificadas'' colapsan en una sola apuesta bajo
  estrés.
\end{description}

\subsection{Especificación del cuadro de mando}
El \emph{dashboard} consume las salidas diarias del detector
(\texttt{prob\_ascending\_channel}, \texttt{prob\_descending\_channel},
\texttt{pred\_canal}, \texttt{confianza}, vía
\texttt{channel\_detector.py predict}) más el libro de posiciones y los
estimadores de volatilidad/correlación. Semáforo: info $\cdot$ aviso $\cdot$
brecha.

\begin{table}[H]
\centering
\footnotesize
\caption{Paneles del cuadro de mando y gatillos.}
\label{tab:dashboard}
\begin{tabularx}{\textwidth}{p{3.4cm} L}
\toprule
\textbf{Panel (capa)} & \textbf{Métricas y gatillos} \\
\midrule
A --- Exposición y \emph{sizing} (Capa 1, diario) &
Volatilidad prevista $\sigma_i$ (EWMA/GARCH); tamaño objetivo $w_i$; ratio
vol.\ realizada/objetivo: aviso $>1.2$ recortar $\cdot$ brecha $>1.5$
\emph{de-risk} 50\%. \\
B --- Límites duros (Capa 2, intradía) &
Utilización de bruto: aviso $>85\%$ $\cdot$ brecha $\geq 100\%$ bloquea nuevas;
sesgo neto por encima de límite $\to$ justificar; concentración: aviso $>25\%$
$\cdot$ brecha $>35\%$ por activo, $>50\%$ por sector; \emph{drawdown}: aviso
$-8\%$ (\emph{de-risk} 30\%) $\cdot$ brecha $-12\%$ (\emph{flat}). \\
C --- Régimen (Capa 3, detector, diario) &
Régimen por activo (\texttt{pred\_canal} + \texttt{confianza}); \% de cartera
contra-régimen ponderado por confianza: aviso $>30\%$ $\to$ revisión del
\emph{desk}; cambio de régimen (confianza $>0.7$ en $\geq 2$ días) $\to$
recalcular VaR/correlaciones; confianza media $<0.5$ $\to$ no accionar. \\
D --- Cartera y correlación (Capa 4, diario) &
VaR/ES 1d 99\%: aviso VaR $>$ límite $\cdot$ brecha $>2$ excepciones de ES en 20
días; correlación media móvil 60d: aviso $\bar\rho>0.6$; ratio de
diversificación: brecha $<1.3$; \emph{backtest} de excepciones VaR
(Kupiec/Christoffersen) $\to$ recalibrar si falla. \\
E --- Salud del modelo (diario) &
Edad de la cabeza logística: aviso $>12$ meses $\to$ recalibrar (justificado por
la Sección~\ref{sec:recalib}); frescura del dato: brecha si el último precio
tiene $>3$ días hábiles; deriva de entrada (PSI de la confianza): aviso $>0.2$;
dos relojes: régimen trimestral--anual, volatilidad/correlación diario. \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Gobernanza}
\textbf{Regla de oro}: ningún gatillo del Panel~C \emph{aumenta} capital; el
\emph{sizing} lo manda el riesgo (Paneles A/B), no el patrón. El modelo es una
herramienta de \emph{challenge}: ante la petición de ``más capital porque el
precio rompe un canal ascendente'', la dirección de riesgos dispone de la
evidencia formal (DSR $\approx 0$, Sección~\ref{sec:backtest}) para exigir que el
tamaño se justifique por riesgo y no por chartismo. Para extender el marco a una
cartera multi-\emph{commodity} basta entrenar una cabeza logística por activo con
el mismo enfoque solo-precio. Esta separación descriptor/decisión es coherente
con las expectativas de gobernanza de modelos de BCBS~239 y de la guía de
modelos internos del BCE \cite{bcbs239,ecb2025guide}.
...
constituye ventaja direccional.

El trabajo se completa con dos patas adicionales que cierran el ciclo
descriptivo. La \textbf{segunda} muestra que la persistencia del régimen es
modelable —C-index 0.664 con estabilidad notable en \emph{walk-forward}
($\pm0.007$)— y reproduce, por una vía metodológica independiente, la ausencia de
valor económico: se acierta el 67.8\% de las direcciones de ruptura y la
estrategia pierde dinero (DSR 0.014), porque \emph{la banda de salida y el
beneficio no son la misma variable}. La \textbf{tercera} traslada ambas señales a
la medición del riesgo y delimita con precisión dónde aportan y dónde no. Sobre
una cartera de seis materias primas, la fragilidad del canal \emph{no} anticipa
la cola —en el quintil de mayor fragilidad ocurren cero excepciones, frente a un
\emph{lift} de 3.19 de la volatilidad EWMA— y una constante del mismo nivel medio
iguala al \emph{overlay}; quien mejora el VaR es el filtrado de volatilidad, con
una cobertura del 1.005\% y un ahorro de capital del 1.8\%. La geometría sí
demuestra valor medible en dos ámbitos distintos del que se le presuponía: la
\textbf{calidad de dato}, donde detecta defectos estructuralmente invisibles a un
control convencional de cola —un precio de liquidación negativo y un relleno de
calendario que juntos distorsionaban 74.4 puntos porcentuales la contribución al
riesgo— y la \textbf{descripción interpretable del régimen}, mediante una cadena
de Markov que supera al modelo i.i.d. y una señal de proximidad al borde que
anticipa la ruptura con seis sesiones de antelación.

El valor del proyecto es, por tanto, triple. Primero, un componente de chartismo
automatizado listo para integrarse en procesos de riesgo como descriptor de
régimen y como control de calidad de dato. Segundo, una demostración
metodológicamente rigurosa —validación cruzada purgada, censura administrativa,
DSR, PBO, controles de nivel y ablaciones— de los límites reales del chartismo,
sostenida por tres vías independientes que convergen en el mismo veredicto.
Tercero, y no menor, un ejemplo documentado de \emph{cómo} se detecta y corrige
una conclusión errónea: los tres defectos de datos de la
Sección~\ref{sec:defectos} habían producido un resultado positivo que no
resistió el escrutinio, y su corrección forma parte del aporte metodológico. La
conclusión transversal es que el sistema debe usarse como \textbf{descriptor de
régimen} —para dimensionar riesgo, horizontes y capital, y para auditar el dato
que los alimenta— y nunca como señal direccional ni como predictor de la cola.
...
% =============================================================================
\section{Hoja de ruta: mejoras propuestas}
\label{ap:mejoras}
% =============================================================================
Priorización tras la revisión completa del proyecto (versión extendida en
\texttt{docs/MEJORAS\_PROPUESTAS.md}).

\subsection{Investigación}
\begin{enumerate}
  \item \textbf{Ampliar el etiquetador geométrico} (triángulos, cuñas,
    banderines, rectángulos, taza-con-asa): es el cuello de botella real de la
    ``capa 2'' del chartismo (Sección~\ref{sec:multipatron}), con
    preentrenamiento sintético de las nuevas clases sobre la infraestructura de
    plantillas existente.
  \item \textbf{Restricción de orden en las salidas continuas} de la CNN
    multitarea ($\widehat{y}_{\mathrm{low}} \leq \widehat{y}_{\mathrm{ret}} \leq
    \widehat{y}_{\mathrm{high}}$, p.\,ej.\ vía \emph{softplus}): elimina las
    inversiones soporte/resistencia observadas.
  \item \textbf{Extensión trimestral del OOS real} (jul--sep 2026 en adelante)
    con FRED como fuente única, acumulando evidencia de estabilidad a coste
    marginal mínimo.
  \item \textbf{Calibración de probabilidades} (Platt/isotónica sobre validación
    purgada) con curvas de fiabilidad: para el uso en riesgo la probabilidad
    debe estar calibrada, no solo ordenar bien.
  \item \textbf{Baseline no-CNN de control}: regresión logística sobre las
    \emph{features} geométricas simples (pendiente, $R^2$, número de extremos) ya
    calculadas por el sistema; blinda la elección del \emph{embedding}
    convolucional ante el comité.
  \item \textbf{Módulo de volatilidad dedicado} (GARCH/HAR-RV/persistencia) para
    la Capa~1 del marco de riesgo: la volatilidad es \emph{forecastable} pero
    este \emph{pipeline} de imágenes no la captura.
\end{enumerate}

\subsection{Ingeniería}
\begin{enumerate}
  \item Tests mínimos (etiquetador sobre ventanas sintéticas conocidas;
    invariantes de purga/embargo; determinismo de la codificación) e integración
    continua ligera.
  \item Tratar el YAML como fuente única de configuración (el JSON duplicado,
    hoy en sincronía verificada, es un riesgo de deriva).
  \item Extraer la rama TensorFlow, no ejercitada por ningún resultado, a un
    directorio \texttt{contrib/} en la versión de revista.
  \item Regenerar \texttt{demo\_predictions.csv} con el nuevo calendario de días
    hábiles (la versión entregada hereda fines de semana del reindexado
    antiguo).
\end{enumerate}
