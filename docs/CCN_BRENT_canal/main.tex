...
independiente. Una tercera pata lleva ambas señales a la medición de riesgo: en
una cartera equiponderada de seis materias primas, con el régimen de cada activo
estimado \emph{solo con su propio precio}, el VaR condicionado alcanza una tasa
de excepciones del 1.087\% frente al objetivo del 1\% y supera tanto al VaR
histórico —que el test de Christoffersen rechaza por agrupar las excepciones—
como al filtrado EWMA sin información de canal. El documento cierra con el marco
...
\subsection{Cartera multi-commodity}
...
precios; la fragilidad de cartera es la media ponderada. El \emph{backtest} cubre
2.024 sesiones (2020-08 a 2026-03) con Kupiec, Christoffersen y semáforo de
Basilea (Tabla~\ref{tab:parte3}, Figura~\ref{fig:parte3}).

\begin{table}[H]
\centering
\footnotesize
\caption{VaR 99\% de la cartera multi-commodity: cobertura, independencia y coste.}
\label{tab:parte3}
\begin{tabular}{lrrrr}
\toprule
\textbf{Estimador} & \textbf{Excepciones} & \textbf{Kupiec $p$} & \textbf{Christoffersen $p$} & \textbf{VaR medio} \\
\midrule
Histórico & 1.186\% & 0.41 & \textbf{0.032} (rechaza) & 3.56\% \\
FHS-EWMA (sin canal) & 1.186\% & 0.41 & 0.289 & 3.85\% \\
\textbf{Predicho (con canal)} & \textbf{1.087\%} & \textbf{0.70} & 0.238 & 4.15\% \\
\midrule
\multicolumn{5}{l}{\footnotesize Objetivo: 1.00\%. Cartera equiponderada de 6 commodities, 2.024 sesiones de test.} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
    ...
    \caption{Tercera pata. Izquierda: cobertura frente al objetivo del 1\%.
    Centro: el VaR histórico agrupa las excepciones en los episodios de estrés y
    el test de independencia lo rechaza; el filtrado EWMA lo corrige. Derecha:
    vida mediana del canal por activo, estimada en cada caso con su propio precio.}
    \label{fig:parte3}
\end{figure}

\subsection{Qué aporta cada componente}
El resultado relevante no es que el VaR predicho gane, sino \emph{por qué}: cada
componente corrige una patología distinta, y reportar solo la cobertura ocultaría
la mitad de la historia.

\begin{itemize}
  \item El \textbf{filtrado EWMA} no mejora la cobertura (1.186\% en ambos casos)
    pero corrige la \textbf{agrupación} de excepciones: el test de Christoffersen
    rechaza el VaR histórico ($p=0.032$) porque concentra los fallos en los
    episodios de estrés, y pasa a $p=0.289$ al filtrar por volatilidad.
  \item El \textbf{\emph{add-on} de canal} corrige el \textbf{nivel}: lleva la
    tasa de excepciones de 1.186\% a 1.087\%, la más próxima al objetivo del 1\%,
    con Kupiec $p=0.70$.
\end{itemize}

Ambas mejoras son complementarias, no sustitutivas.

\subsection{El coste en capital: las excepciones también consumen}
Sería incompleto medir el coste del \emph{overlay} únicamente por el nivel del
VaR. Bajo modelos internos el capital por riesgo de mercado es proporcional a
$k \cdot \mathrm{VaR}$, donde $k$ es el multiplicador del semáforo de Basilea,
que \emph{crece con el número de excepciones} de \emph{backtesting} (3.00 en zona
verde; 3.40--3.85 en amarilla; 4.00 en roja). Reducir excepciones ahorra capital,
de modo que el balance debe calcularse completo. Tomando el multiplicador que
correspondería a la \emph{peor ventana móvil de 250 sesiones} —la que determina
el recargo en un momento dado— se obtiene la Tabla~\ref{tab:capital}.

\begin{table}[H]
\centering
\footnotesize
\caption{Capital regulatorio aproximado ($k\cdot\mathrm{VaR}$) de los tres estimadores.}
\label{tab:capital}
\begin{tabular}{lrlrrr}
\toprule
\textbf{Estimador} & \textbf{Peor 250d} & \textbf{Zona} & \textbf{$k$} & \textbf{VaR medio} & \textbf{Capital} \\
\midrule
Histórico & 7 exc. & amarilla & 3.65 & 3.56\% & 13.00\% \\
FHS-EWMA & 6 exc. & amarilla & 3.50 & 3.85\% & 13.48\% \ (+3.6\%) \\
Predicho & 6 exc. & amarilla & 3.50 & 4.15\% & \textbf{14.51\%} \ (+11.6\%) \\
\bottomrule
\end{tabular}
\end{table}

El \emph{overlay} sí rebaja el recargo ($k$ de 3.65 a 3.50), pero ese ahorro
\textbf{no compensa} el mayor nivel de VaR: en términos de capital cuesta un
\textbf{+11.6\%} frente al histórico. La conclusión honesta es que el valor del
VaR predicho \textbf{no está en la eficiencia de capital} sino en la
\textbf{validación y gobernanza del modelo}: el VaR histórico \emph{falla} el
test de independencia ($p=0.032$) —un hallazgo supervisor en sí mismo, porque
concentra los fallos justo en los episodios de estrés— mientras que el predicho
supera ambos contrastes con la cobertura más próxima al objetivo. Si el criterio
fuese minimizar capital, la elección sería el VaR histórico, asumiendo el riesgo
de que su agrupación de excepciones provoque una migración de zona en el
siguiente episodio de tensión.

Las cinco aplicaciones implementadas —calidad de dato, previsión de régimen de
volatilidad, VaR de un activo, VaR de cartera y proxies FRTB— se ejecutan bajo un
\emph{harness} reproducible que registra por corrida un manifiesto con
identificadores, configuración, semilla, comando, entorno, métricas, baseline y
decisión, de acuerdo con la disciplina de \emph{harness engineering} adoptada en
el proyecto.
...
beneficio no son la misma variable}. La \textbf{tercera} demuestra dónde sí
aporta valor la señal: en la medición del riesgo. Sobre una cartera de seis
materias primas, con el régimen de cada activo estimado solo con su propio
precio, el VaR condicionado por la fragilidad del canal alcanza la mejor
cobertura de los tres estimadores contrastados (1.087\% frente al objetivo del
1\%) y corrige la patología del VaR histórico, cuyas excepciones se agrupan en
los episodios de estrés.

El valor del proyecto es, por tanto, triple: un componente de chartismo
automatizado listo para integración en procesos de riesgo; una demostración
metodológicamente rigurosa —CV purgada, censura administrativa, DSR, PBO— de los
límites reales del chartismo como fuente de \emph{alpha}, sostenida ahora por dos
metodologías independientes; y una aplicación con mejora medible allí donde la
geometría sí es informativa, la calibración del riesgo de cola. La conclusión
transversal es que el sistema debe usarse como \textbf{descriptor de régimen}
—para dimensionar riesgo, horizontes y capital— y nunca como señal direccional.
