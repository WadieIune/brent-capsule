...
def load_observed_prices(cfg: Dict[str, object], seed: int) -> Tuple[pd.Series, str]:
    ...
    return series, f"file:{os.path.basename(str(path))} (sesiones observadas, sin relleno)"
...
def _threshold_reviews(
    ...
    """Revisa cada sesión cuya probabilidad supera el umbral."""
    return np.flatnonzero(mask & np.isfinite(scores) & (scores >= threshold))
    ...
    metrics = {
        "protocol": {
            ...
            "independence_incident": (
                "El worktree compartido cambió accidentalmente a un commit de Track A "
                "antes del run real. Solo se vio su veredicto final, no sus métricas; "
                "el diseño y el código de Track B ya estaban fijados. Sin efecto en "
                "umbrales, modelos o interpretación de WP2-B."
            ),
            "threshold_selection": (
